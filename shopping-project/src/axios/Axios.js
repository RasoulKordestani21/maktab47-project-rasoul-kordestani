...
export const ReceivedProduct = (id) => {
  ...
  console.log(id, time);
  try {
    response = axios.patch(`${baseURL}/orders/${id}`, {
      dateOfReceive: time,
      isReceived: true,
    });
  } catch (error) {
...
export const getDairy = (start = 3, limit = 5) => {
  ...
  try {
    const one = 1;
    response = axios.get(
      `http://localhost:3000/dairy?_start=${start}&_limit=${limit}`
    );
  } catch (error) {
    console.error(error);
  }
...
export const getHealthWashing = async (a = 3, b = 5) => {
  ...
  try {
    ...
    response = await axios.get(
      `http://localhost:3000/health-Washing?_start=${a}&_limit=${b}`
    );
    // console.log(response);
  } catch (error) {
    // console.error(error);
...
export const postUserData = async (userData) => {
  await axios
    .post("http://localhost:3000/userSpecifics", {
