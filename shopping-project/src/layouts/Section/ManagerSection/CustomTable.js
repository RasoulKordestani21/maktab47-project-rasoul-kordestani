...
function CustomTable(props) {
  ...
  const calcWholePrice = (arrOfProduct) => {
    ...
    let wholePrice = 0;
    arrOfProduct.forEach(ele =>
      wholePrice = Number(ele.price) * ele.numOfOrd + wholePrice)
    return wholePrice
  }
  ...
  useEffect(() => {
    ...
    if (!props.isFiltered) {
      getOrders().then(res => { setState(res.data) })
    }
    else {
      
      getOrders().then(res => { console.log(res.data.filter(ele=>ele.isReceived ==props.isReceived));setState(res.data.filter(ele => ele.isReceived == props.isReceived)) })
    }
  ...
  return (
    <>
      <TableContainer style={{ height:'400px', width: "80%", margin: "auto" }} component={Paper}>
      ...
      <div style={{ width: "50%", margin: "auto", padding: '20px' }}>
        <div style={{ direction: "ltr" }}>
          <Pagination classes={{ ul: classes.ul }} count={4} color="primary" page={page} onChange={handleChange} />
        </div>
      </div>
    </>
  );
