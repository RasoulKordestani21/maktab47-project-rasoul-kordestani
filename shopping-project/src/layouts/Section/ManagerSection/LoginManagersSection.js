...
class LoginManagersPage extends Component {
  ...
  login = () => {
    ...
    if (
      this.state.username == this.state.adminData.username &&
      this.state.password == this.state.adminData.password
    ) {
  ...
  render() {
    ...
    const avatarStyle = {
      marginTop: 20,
      marginBottom: 40,
      backgroundColor: "rgb(101, 72, 182)",
    };
    ...
    const errorMessage = {
      fontSize: 14,
      color: "red",
      visibility: this.state.errorFlag ? "visible" : "hidden",
    };
